...
class Game(ABC):
    ...
    def display_level_complete(self, score, is_multiplayer, has_won=None):
        if is_multiplayer:
            if has_won:
                text = "YOU LOOSE"
            else:
            ...
            text = "LEVEL COMPLETE"
        message = ScreenText(os.path.join("assets", "PixelIntv-OPxd.ttf"), text, "#735737", 30)
        score_text = ScreenText(os.path.join("assets", "PixelIntv-OPxd.ttf"), f"score: {score}", "#735737", 30)
        select_level_btn = TextButton(200, 400, 200, 80, "select level", 20)
        back_to_menu_btn = TextButton(200, 500, 200, 80, "back to menu", 20)

        pygame.draw.rect(self.screen, "#4e4e94", pygame.Rect(135, 105, 530, 414))

        message.draw(self.screen, 150, 150)
        score_text.draw(self.screen, 150, 200)
