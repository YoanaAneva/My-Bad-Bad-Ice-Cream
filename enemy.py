...
class Enemy(pygame.sprite.Sprite):
    """Preserve the information about an ememy"""
    ...
    def curr_board_cell(self) -> Tuple[int, int]:
        return ((self.rect.center[1] - FRAME_DIMS[0]) // 44, (self.rect.center[0] - FRAME_DIMS[1]) // 44)
    ...
    def calculate_target(self, player: Player) -> Tuple[int, int]:
        """Depending on the enemy name calculate its target coordinates"""
        # blinki chaces the player from behind
        if self.name == "Blinki":
            return player.rect.center

        # pinki's target is 10 pixels ahead of the player rect
        if self.name == "Pinki":
            if player.direction == "right":
                if player.rect.center[0] + 10 > SCREEN_DIMS[0] - FRAME_DIMS[0]:
                    return (SCREEN_DIMS[0] - FRAME_DIMS[0] - player.rect.center[0], player.rect.center[1])
                else:
                    return (player.rect.center[0] + 10, player.rect.center[1])
            
            if player.direction == "left":
                if player.rect.center[0] - 10 <= FRAME_DIMS[0]:
                    return (FRAME_DIMS[0], player.rect.center[1])
                else:
                    return (player.rect.center[0] - 10, player.rect.center[1]) 
                
            if player.direction == "back":
                if player.rect.center[1] - 10 <= FRAME_DIMS[1]:
                    return (player.rect.center[0], FRAME_DIMS[1])
                else:
                    return (player.rect.center[0], player.rect.center[1] - 10)
            
            if player.direction == "front":
                if player.rect.center[1] + 10 >= SCREEN_DIMS[1] - FRAME_DIMS[1]:
                    return (player.rect.center[0], SCREEN_DIMS[1] - FRAME_DIMS[1] - player.rect.center[1])
                else:
                    return (player.rect.center[0], player.rect.center[1] + 10)
    ...
    def decide_next_move(self, player: Player, board: List[int]) -> str:
        ...
        self.visited_points.append(next_step)
        if len(self.visited_points) > MIN_CYCLE_LEN:
    ...
    def is_recently_visited(self, step: Tuple[int, int]) -> bool:
        """Check if a point on the board is in the list"""
        for coordinates in self.visited_points:
            if coordinates == step:
                return True
        return False

def calculate_distance_to_target(enemy_center, player_center):
    """Calculate the distance between player's rect and enemy"""
    x1 = enemy_center[0]
    y1 = enemy_center[1]
    x2 = player_center[0]
    y2 = player_center[1]
